...
def listen_kafka(bootstrap_servers, output_filename, input_topic, output_topic):
    ...
    for message in consumer:
        ...
        #output_label[uid] = int(np.round(json.loads(','.join(line.split(',')[1:])[1:-1].replace('""','"'))['data'][0]))
        label_get = json.loads(','.join(line.split(',')[1:])[1:-1].replace('""','"'))['data'][0]
        if label_get>=0.5:
            label_get=int(1)
        else:
            label_get=int(0)
        output_label[uid] = label_get
